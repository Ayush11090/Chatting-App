...
const ChatPage = () => {
  ...
  const sendAudioToPredictionServer = (audioFile: File): Promise<any> => {
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = async () => {
            try {
                const base64Audio = (reader.result as string).split(",")[1]; // Extract Base64 portion
                const payload = {
                    audio: base64Audio,
                    filename: audioFile.name, // Include the filename
                };
                ...
                console.error("Error sending audio file to prediction server:", error);
  ...
  const sendChatMessage = async () => {
    ...
    if (attachedFiles && attachedFiles.length > 0) {
      // Check if there is an audio file and send it to the prediction server
      for (let attachment of attachedFiles) {
        if (attachment.type.startsWith("audio/")) {
          try {
            const predictionResult = await sendAudioToPredictionServer(attachment);
            console.log(predictionResult,12233);
            if (predictionResult.predicted_class == 1) {
              alert("Audio file contains offensive content.");
              return; // Exit early if there's an issue with the audio file
            }
          } catch (error) {
            console.error("Error processing audio file for prediction:", error);
            alert("Error processing audio file for prediction.");
            return; // Exit early if there's an issue with the audio file
          }
        }
      }
                    ...
                    return (
                      ...
                      >
                        ...
                        <img
                          ...
                          src="https://www.svgrepo.com/show/7884/attachment.svg"
                          alt="attachment"
                        />
